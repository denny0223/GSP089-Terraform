...
- [`versions.tf`](versions.tf)：設定 Terraform 版本需求、Google provider 版本，以及 provider 使用的專案、區域、可用區。
...
`terraform destroy` 會根據 state 刪除 Terraform 管理的資源。lab 結束前使用它清理 VM、監控設定和 dashboard。
...
每次啟動 Skills Boost lab 時，Google Cloud 專案 ID（project id）可能不同。請先設定本 lab 使用的區域（region）與可用區（zone）：
...
這些設定主要來自 [`main.tf`](main.tf) 裡的 `google_compute_instance.lamp`。HTTP 防火牆規則也會一併建立，對應 Console 建立 VM 時的「允許 HTTP 流量」。
