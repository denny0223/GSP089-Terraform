...
Skills Boost 是本 lab 的入口和判分系統；最後是否完成，以 Skills Boost 的 Check my progress 為準。不過 lab 時間只有 20 分鐘，不建議一打開 README 就啟動 lab。
...
> [!IMPORTANT]
> 先不要啟動 Skills Boost lab。lab 時間只有 20 分鐘，建議先讀到「Terraform 執行流程」，理解 Terraform 會建立哪些資源、每個檔案負責什麼，以及後面指令會做什麼。
...
- 提供者（provider）：告訴 Terraform 要使用哪個雲端平台或服務。本專案使用 `hashicorp/google` provider。
- 資源（resource）：要由 Terraform 建立或管理的物件，例如 VM、防火牆規則、uptime check、alert policy。
- 資料來源（data source）：讀取已存在的資源。本專案會讀取 Google Cloud 預設網路（default network）。
...
看 [`variables.tf`](variables.tf) 時，可以注意每個 `variable` block 通常會包含幾個欄位：`type` 定義資料型別，例如字串；`description` 說明變數用途；`default` 則是沒有另外指定時會使用的預設值。像 `project_id` 沒有預設值，表示每次 lab 都必須提供；`region` 和 `zone` 有預設值，但實際操作時仍要以 Skills Boost 當次指定內容為準。
...
README 只會說明本 lab 需要理解的重點；如果你想知道 `.tf` 語法、resource 支援欄位或 CLI 指令細節，請查官方文件。
...
本 lab 先把 state 留在 Cloud Shell 的本機目錄；同一次 lab 請盡量在同一個 `GSP089-Terraform` 目錄完成，不要中途刪掉 state 或換到全新的目錄。正式專案通常不會把 state 放在 repo，而會放在遠端儲存位置，例如 Cloud Storage。

資源建立後，盡量不要直接在 Google Cloud Console 手動修改 Terraform 管理的 VM、防火牆、uptime check、alert policy 或 dashboard。手動修改會讓 Console 上的實際狀態和 `.tf` 檔案描述的目標狀態不一致；如果要調整設定，回到對應的 `.tf` 檔修改，再重新執行 `terraform plan` 和 `terraform apply`，比較容易知道變更了什麼。
...
後續 `terraform init`、`terraform plan`、`terraform apply` 和清理指令都要在專案目錄中執行。
...
接著用剛才設定的變數產生本次 lab 專用的 `terraform.tfvars`。Terraform 會從該檔案讀取專案、region、zone 和 VM 設定：
...
本專案已經包含 `.terraform.lock.hcl`，所以 `terraform init` 會依照鎖定檔選擇 provider 版本。只有 `.terraform/` 目錄、state 檔案和本次 lab 專用的 `terraform.tfvars` 不需要提交。
...
> [!NOTE]
> 請先看 plan 裡的摘要。本 lab 預期會建立 VM、防火牆規則、uptime check、alert policy、可能的 email notification channel，以及 dashboard。`plan` 還不會真正建立資源。
...
Terraform 會建立 `Lamp Uptime Check`，每 60 秒用 HTTP 檢查 VM 外部 IP。設定來源是 [`monitoring.tf`](monitoring.tf) 裡的 `google_monitoring_uptime_check_config.lamp`。
...
門檻值為大於 `500`，重新測試週期為 `60s`。設定來源是 [`monitoring.tf`](monitoring.tf) 裡的 `google_monitoring_alert_policy.inbound_traffic`。
...
延伸練習會執行 Python 範例程式 [`otel_demo.py`](samples/opentelemetry/otel_demo.py)，產生 traces 和 metrics。接著使用 VM 上的 Ops Agent OTLP receiver 收資料，送到 Cloud Monitoring 和 Cloud Trace。這次操作不示範 OpenTelemetry logs。
...
這份設定會讓 Ops Agent 在 VM 上接收 OTLP gRPC 資料。預設 endpoint 是：
...
點進任一 trace，可以看到 `handle_request` 和子 span 的父子關係，以及每個 span 的耗時。Trace 畫面可以理解成一張請求時間線：越長的 span，代表那段工作花越久。

範例程式會把 route 和 status 記在主要 span `handle_request` 的 attributes 上。要找特定路徑的 trace，可以在 Trace Explorer 使用 Filter bar：
