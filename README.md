...
看 [`variables.tf`](variables.tf) 時，可以注意每個 `variable` block 通常會包含幾個欄位：`type` 定義資料型別，例如字串；`description` 說明這個變數用途；`default` 則是沒有另外指定時會使用的預設值。像 `project_id` 沒有預設值，表示每次 lab 都必須提供；`region` 和 `zone` 有預設值，但實際操作時仍要以 Skills Boost 當次指定內容為準。
...
這個 lab 先把 state 留在 Cloud Shell 的本機目錄；同一次 lab 請盡量在同一個 `GSP089-Terraform` 目錄完成，不要中途刪掉 state 或換到全新的目錄。正式專案通常不會把 state 放在 repo，而會放在遠端儲存位置，例如 Cloud Storage。
